...
export let Go;
export let stdout_stderr = '';

export function initGo(fs, Buffer) {
	var handler = {
	...
	globalThis.fs.writeSync = function (fd, buf) {
		if (fd === 1 || fd === 2) {
			...
			if (nl != -1) {
				stdout_stderr += outputBuf.substring(0, nl + 1);
	...
	globalThis.fs.write = function (fd, buf, offset, length, position, callback) {
		if (fd === 1 || fd === 2) {
			...
			// buf:
			arguments[1] = globalThis.Buffer.from(arguments[1]);
	...
	globalThis.fs.open = function (path, flags, mode, callback) {
		...
		return globalThis.fs.openOriginal(path, myflags, mode, callback);
	};

	globalThis.fs.fstatOriginal = globalThis.fs.fstat;
	globalThis.fs.fstat = function (fd, callback) {
		return globalThis.fs.fstatOriginal(fd, function () {
	...
	globalThis.fs.close = function (fd, callback) {
		return globalThis.fs.closeOriginal(fd, function () {
